% Technical note -- version 2 (2026-09-25, after preprint v1 doi:10.5281/zenodo.22952930):
% abstract ratios and equal-area saving aligned with the tables; transition criterion of Table 1
% stated (Michel) with the e^N sensitivity; references for prior shape optimisation and methods.
% v1 source: nota_tecnica_v1_zenodo.tex.
% Draft 3 (2026-09-25): English figures, guaranteed-shape section,
% 2D optimisation dropped, references completed. Draft 2: reference-propeller sensitivity added
...
at fixed frontal area and $L/D\approx6$ at fixed volume; with a turbulent boundary layer,
axisymmetric golden-ratio bodies have 2.4--3.6 times the drag of the optimum (a sphere 3.8--7.5
times), in agreement with Hoerner's classical data and with earlier optimisation studies.
...
body pushed by an ideal propeller of infinite disc area, and $15\%$ less than with an
...
\cite{hoerner1965}, and minimum-drag axisymmetric bodies have been optimised with
panel/boundary-layer methods before \cite{parsons1974,myring1976,lutz1998}; we re-derive it with reproducible tools and quantify how far the
...
\cite{smith1993,drela2009,dellacorte2021}. Its theoretical benefit is clear, but
...
A D2Q9 lattice-Boltzmann solver \cite{kruger2017} (\texttt{lbm.py}) was used at $Re=200$ (based on body
height $H$, 30 lattice units) to compare a circle, golden-ratio shapes (ellipse, rectangle,
teardrop, golden spiral) and teardrops of $L/H=3$ and $4.5$. This 2D step is a qualitative
screening only.

\subsection{Axisymmetric reduced-order model}
The method follows the classical approach of \cite{young1939,myring1976,lutz1998}
(\texttt{axi.py}): the inviscid flow is computed with an axisymmetric panel method (vortex
rings on the surface); the laminar boundary layer with Thwaites' method \cite{thwaites1949}
in its axisymmetric form; transition with the $e^N$ envelope method of Drela and Giles
\cite{drela1987} or with Michel's criterion \cite{michel1951}; the turbulent boundary layer with
Head's entrainment method \cite{head1958} and the Ludwieg--Tillmann skin-friction law
\cite{ludwieg1950}; separation is detected from Thwaites' parameter ($\lambda<-0.09$, laminar) or the shape factor
($H>2.4$, turbulent), and a separated afterbody receives a base-drag estimate. The total drag follows from
Young's formula \cite{young1939} applied to the momentum thickness near the tail.
Bodies are described by five parameters (fineness ratio and four class/shape-function
coefficients \cite{kulfan2008}). The objective is the volumetric drag coefficient
$C_{D,V}=D/(\tfrac12\rho U^2V^{2/3})$ (fixed volume) or the drag based on frontal area.
Optimisation uses differential evolution \cite{storn1997} (\texttt{axi\_opt.py}).
To test how much a shape designed for natural transition depends on the uncertain transition
location, six scenarios are defined (\texttt{robusta.py}): $e^N$ transition with free-stream
turbulence $Tu=0.05$, $0.1$, $0.3$ and $1\%$, Michel's criterion, and a ``rough'' case with
transition forced at $Re_x=2\times10^6$; transition is in any case capped at $Re_x=5\times10^6$
($10^7$ for $Tu=0.05\%$). At fixed volume and power the speed scales as $C_{D,V}^{-1/3}$.
Three shapes are compared at each $\ReV$: the turbulent optimum (reference), the
\emph{nominal} optimum for $Tu=0.1\%$, and a \emph{guaranteed} shape that maximises the
smallest speed gain over the six scenarios.
...
$1.08$--$1.39$ (circle: $1.79$), against $0.73$ for the $L/H=3$ teardrop and $0.566$ for
the $L/H=4.5$ teardrop, i.e.\ 1.9--2.5 times the best 2D shape. (A 2D shape optimisation
was also run, but at a coarser lattice than the comparison shapes; its result is not
comparable and is not reported.)

In axisymmetric flow (Table~\ref{tab:shape}, Fig.~\ref{fig:opt3d}) the optimum at fixed
volume and turbulent boundary layer has $L/D=6.6$ ($\ReV=10^7$). At fixed frontal
area the optimum is $L/D=3.2$. With a turbulent boundary layer, golden-ratio bodies
($L/D=1.618$) separate on the afterbody and have 2.4--3.6 times the drag of the optimum; a
sphere has 3.8--7.5 times. These values are consistent with Hoerner's compilation
\cite{hoerner1965} and with earlier optimisations \cite{parsons1974,lutz1998}.

The natural-transition rows of Table~\ref{tab:shape} were computed with Michel's criterion
and the cap $Re_x\le5\times10^6$; they give $L/D=6.0$ at $\ReV=10^7$ and $L/D=9.3$ at
$\ReV=10^6$, the latter with laminar flow up to $x/L=0.87$. They are optimistic and should be read
as an upper bound: re-evaluated with the $e^N$ criterion ($Tu=0.1\%$), the same $\ReV=10^6$ body
has $C_{D,V}=0.0180$ instead of $0.0072$, while the $\ReV=10^7$ body is unchanged ($0.0165$).
The effect of the transition criterion on the shape is quantified in the next section, where
the nominal $e^N$ optimum at $\ReV=10^6$ has $L/D=5.6$.

\begin{table}[h]\centering
\caption{Axisymmetric shape optimisation (reduced-order model). $C$: drag coefficient on the
optimisation reference (volume$^{2/3}$ or frontal area). ``Natural'': transition by Michel's
criterion, $Re_x\le5\times10^6$ (optimistic, see text).}\label{tab:shape}
\small\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcccccc}
\toprule
Case & $L/D_{\rm opt}$ & $C_{\rm opt}$ & gold.\ ellipsoid & gold.\ teardrop & sphere & $L/D=4.5$\\
\midrule
...
\subsection{Robustness to the transition location}
Table~\ref{tab:robusta} and Fig.~\ref{fig:robusta} give the speed change at equal power
relative to the turbulent optimum. At $\ReV=10^6$ the nominal shape ($L/D=5.6$) is 16--18\%
faster in calm conditions but 6--7\% slower when the free-stream turbulence is $0.3\%$ or more
or the surface is rough: its gain depends on a long laminar run that disturbances destroy. The
guaranteed shape ($L/D=4.0$) is faster in every scenario, but only by 2.7--3.6\%. At
$\ReV=3\times10^6$ the guaranteed gain is $0.4\%$, and from $\ReV=10^7$ on all shapes coincide
with the turbulent optimum within $0.6\%$, because the laminar run is a small fraction of the
length. These numbers are optimistic in three respects: in some scenarios transition is
triggered by laminar separation, with no penalty for the separation bubble; the cap on the
laminar run is an assumption that decides the result at high $\ReV$; and the nominal optimum at
$\ReV=3\times10^6$ ($L/D=2.9$) is probably a local minimum. The robust conclusion is the
qualitative one: a laminar-flow shape can gain much or lose much, and the gain that can be
guaranteed is small.

\begin{table}[h]\centering
\caption{Speed change at equal power relative to the turbulent optimum, range over the six
transition scenarios (reduced-order model, \texttt{robusta.py}).}\label{tab:robusta}
\begin{tabular}{lcccc}
\toprule
$\ReV$ & nominal $L/D$ & nominal shape & guaranteed $L/D$ & guaranteed shape\\
\midrule
$10^6$ & 5.6 & $-6.9$ to $+17.7\%$ & 4.0 & $+2.7$ to $+3.6\%$\\
$3\times10^6$ & 2.9 & $-4.3$ to $+12.0\%$ & 5.3 & $+0.4$ to $+4.2\%$\\
$10^7$ & 6.0 & $-0.5$ to $+0.6\%$ & 6.5 & $0.0$ to $+0.2\%$\\
$3\times10^7$ & 6.5 & $0.0\%$ & 6.5 & $0.0\%$\\
\bottomrule
\end{tabular}
\end{table}
...
area, $\approx6$ at fixed volume); golden-ratio shapes have 2.4--3.6 times the drag in
axisymmetric turbulent flow (1.9--2.5 times in the 2D screening).
\item Shapes optimised for a long laminar run are fragile: with natural transition at
$\ReV=10^6$ they can be 18\% faster or 7\% slower than the turbulent optimum depending
on the free-stream disturbances; a shape designed for the worst case guarantees only
about 3\%, and nothing above $\ReV\approx10^7$.
...
(archived version v1.0: \href{https://doi.org/10.5281/zenodo.22949389}{doi:10.5281/zenodo.22949389};
all versions: \href{https://doi.org/10.5281/zenodo.22949388}{doi:10.5281/zenodo.22949388}). Table~\ref{tab:files} maps each figure and
...
Table~\ref{tab:robusta}, Fig.~\ref{fig:robusta} & \texttt{robusta.py} $\rightarrow$ \texttt{plot\_robusta.py}\\
...
\includegraphics[width=\linewidth]{resistenza_Re200_en.png}
\caption{2D lattice-Boltzmann drag at $Re=200$ for golden-ratio shapes and teardrops.}
\label{fig:lbm}
\end{figure}
\begin{figure}[p]\centering
\includegraphics[width=\linewidth]{ottimo_3D_en.png}
\caption{Axisymmetric optimal bodies.}\label{fig:opt3d}
\end{figure}
\begin{figure}[p]\centering
\includegraphics[width=\linewidth]{robusta_en.png}
\caption{Nominal (optimised for Tu $=0.1\%$) and guaranteed (max--min) shapes with natural
transition, and their speed change at equal power relative to the turbulent optimum in six
transition scenarios.}\label{fig:robusta}
\end{figure}
\begin{figure}[p]\centering
\includegraphics[width=0.9\linewidth]{mappa_finale_en.png}
...
shown for information only (not a result).}\label{fig:mappa}
\end{figure}
\begin{figure}[p]\centering
\includegraphics[width=\linewidth]{cfd_conv_1e6f_en.png}
\caption{RANS versus reduced-order model, bare body at $\ReV=10^6$.}\label{fig:cfdconv}
\end{figure}
\begin{figure}[p]\centering
\includegraphics[width=0.9\linewidth]{carena_flusso_en.png}
...
\bibitem{parsons1974} J.~S. Parsons, R.~E. Goodson, F.~R. Goldschmied, Shaping of axisymmetric
bodies for minimum drag in incompressible flow, \emph{J. Hydronautics} 8(3):100--107, 1974,
doi:10.2514/3.48131.
\bibitem{myring1976} D.~F. Myring, A theoretical study of body drag in subcritical axisymmetric
flow, \emph{Aeronautical Quarterly} 27(3):186--194, 1976.
\bibitem{lutz1998} Th.~Lutz, S.~Wagner, Drag reduction and shape optimization of airship bodies,
\emph{J. Aircraft} 35(3):345--351, 1998, doi:10.2514/2.2313.
\bibitem{young1939} A.~D. Young, The calculation of the total and skin friction drags of bodies
of revolution at zero incidence, ARC R\&M 1874, 1939.
\bibitem{thwaites1949} B.~Thwaites, Approximate calculation of the laminar boundary layer,
\emph{Aeronautical Quarterly} 1:245--280, 1949.
\bibitem{drela1987} M.~Drela, M.~B. Giles, Viscous-inviscid analysis of transonic and low
Reynolds number airfoils, \emph{AIAA J.} 25(10):1347--1355, 1987, doi:10.2514/3.9789.
\bibitem{michel1951} R.~Michel, \'Etude de la transition sur les profils d'ailes; \'etablissement
d'un crit\`ere de d\'etermination du point de transition et calcul de la tra\^in\'ee de profil
incompressible, ONERA Rapport 1/1578A, 1951.
\bibitem{head1958} M.~R. Head, Entrainment in the turbulent boundary layer, ARC R\&M 3152, 1958.
\bibitem{ludwieg1950} H.~Ludwieg, W.~Tillmann, Investigations of the wall-shearing stress in
turbulent boundary layers, NACA TM 1285, 1950.
\bibitem{kulfan2008} B.~M. Kulfan, Universal parametric geometry representation method,
\emph{J. Aircraft} 45(1):142--158, 2008, doi:10.2514/1.29958.
\bibitem{storn1997} R.~Storn, K.~Price, Differential evolution -- a simple and efficient
heuristic for global optimization over continuous spaces, \emph{J. Global Optimization}
11:341--359, 1997, doi:10.1023/A:1008202821328.
\bibitem{kruger2017} T.~Kr\"uger, H.~Kusumaatmaja, A.~Kuzmin, O.~Shardt, G.~Silva, E.~M. Viggen,
\emph{The Lattice Boltzmann Method: Principles and Practice}, Springer, 2017,
...
1966; \emph{J. Hydronautics} 1(1):2--11, 1967, doi:10.2514/3.62746.
...
Power} 9(1):74--82, 1993, doi:10.2514/3.11487.
\bibitem{drela2009} M.~Drela, Power balance in aerodynamic flows, \emph{AIAA J.} 47(7):1761--1771, 2009, doi:10.2514/1.42409.
\bibitem{dellacorte2021} B.~Della~Corte, M.~van~Sluis, L.~L.~M. Veldhuis, A.~Gangoli~Rao,
Power balance analysis experiments on an axisymmetric fuselage with an integrated
boundary-layer-ingesting fan, \emph{AIAA J.} 59(12):5211--5224, 2021, doi:10.2514/1.J060570.
\bibitem{menter1994} F.~R. Menter, Two-equation eddy-viscosity turbulence models for
engineering applications, \emph{AIAA J.} 32(8):1598--1605, 1994, doi:10.2514/3.12149.
\bibitem{langtry2009} R.~B. Langtry, F.~R. Menter, Correlation-based transition modeling
for unstructured parallelized CFD codes, \emph{AIAA J.} 47(12):2894--2906, 2009,
doi:10.2514/1.42362.
\end{thebibliography}
\end{document}
